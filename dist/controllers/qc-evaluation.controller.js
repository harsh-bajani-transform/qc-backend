...
exports.submitQCEvaluation = exports.getQCEvaluationData = exports.getQCFileDetails = exports.getQCFilesForEvaluation = void 0;
const db_1 = __importDefault(require("../database/db"));
const crypto_1 = __importDefault(require("crypto"));
const user_queries_1 = __importDefault(require("../queries/user-queries"));
const qc_queries_1 = __importDefault(require("../queries/qc-queries"));
...
const getQCFilesForEvaluation = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        try {
            ...
            const user = yield user_queries_1.default.getUserWithDesignation(user_id);
            if (!user) {
            ...
            const qcFiles = yield qc_queries_1.default.getQCFilesForEvaluation();
            // Group files by project for better organization
            const filesByProject = qcFiles.reduce((acc, file) => {
            ...
            const evaluatedFiles = qcFiles.filter((file) => file.qc_score !== null).length;
            const pendingFiles = totalFiles - evaluatedFiles;
            // Get role hierarchy info
            const allRoles = yield user_queries_1.default.getAllDesignations();
            const accessibleRoles = allRoles.filter((role) => role.designation_id >= 1 && role.designation_id <= 5);
            const inaccessibleRoles = allRoles.filter((role) => role.designation_id < 1 || role.designation_id > 5);
...
exports.getQCFilesForEvaluation = getQCFilesForEvaluation;
// Get specific file details for QC evaluation
const getQCFileDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        try {
            ...
            const qcFile = yield qc_queries_1.default.getQCFileDetails(qc_id);
            if (!qcFile) {
                return res.status(404).json({
                    success: false,
            ...
            const [recordStats] = yield connection.execute(`SELECT 
        ...
        WHERE project_id = ? 
        AND user_id = ?`, [file.project_id, file.user_id] // We need to get the original user_id from qc_performance
            );
...
exports.getQCFileDetails = getQCFileDetails;
// Get QC evaluation data for a specific file
const getQCEvaluationData = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        ...
        const evaluationCriteriaRaw = (_a = req.body.evaluation_criteria) !== null && _a !== void 0 ? _a : req.body.criteria;
        ...
        try {
         ...
         ORDER BY tr.created_at DESC`, [project_id]);
            ...
            const aiInputRecords = evaluationData
                ...
                .map((r) => (Object.assign({ id: r.record_id }, r.record_data)));
            let ai_analysis = null;
            let ai_feedback = null;
            const ai = {
                enabled: true,
                configured: ai_1.default.isConfigured(),
                ran: false,
            ...
            if (!ai.configured) {
                ...
                try {
                    ai.ran = true;
                    const aiResults = yield ai_1.default.analyzeRecords(aiInputRecords, evaluationCriteria);
                    const scores = Array.isArray(aiResults)
                        ? aiResults.map((x) => { var _a; return Number((_a = x === null || x === void 0 ? void 0 : x.score) !== null && _a !== void 0 ? _a : 0); }).filter((n) => Number.isFinite(n))
                        : [];
                    ...
                    if (overallScore > 0 && overallScore < AI_THRESHOLD) {
                        const recordsWithIssues = Array.isArray(aiResults)
                            ? aiResults.filter((x) => { var _a; return ((x === null || x === void 0 ? void 0 : x.isCompliant) === false) || Number((_a = x === null || x === void 0 ? void 0 : x.score) !== null && _a !== void 0 ? _a : 0) < 95; }).length
                            : 0;
                        ai_feedback = yield ai_1.default.generateEvaluationFeedback({
...
exports.getQCEvaluationData = getQCEvaluationData;
// Submit QC evaluation results
const submitQCEvaluation = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { user_id, project_id, evaluation_results, overall_notes } = req.body;
        ...
        try {
            ...
            const [userCheck] = yield connection.execute('SELECT u.user_id, u.user_name, u.role_id FROM tfs_user u WHERE u.user_id = ? AND u.role_id >= 3', [user_id]);
            if (userCheck.length === 0) {
                return res.status(403).json({
                    success: false,
            ...
            const passedCount = evaluation_results.filter((result) => result.evaluation_status === 'passed').length;
           ...
           WHERE id IN (
             SELECT id FROM (
               ...
               LIMIT 1
                ...
                const evaluationId = crypto_1.default.randomUUID();
                yield connection.execute(`INSERT INTO qc_record_evaluations 
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            const evaluationId = crypto_1.default.randomUUID();
            yield connection.execute(`INSERT INTO qc_evaluations 
            ...
            if (passRate < 95) {
                try {
                    const evaluationData = {
                        ...
                        issuesByRecord: evaluation_results.filter((result) => result.evaluation_status === 'failed').map((result) => ({
                            recordId: result.record_id,
                            issues: result.issues || ['Evaluation failed'],
                            score: result.evaluation_score || 0
                        }))
                    };
                    aiFeedback = yield ai_1.default.generateEvaluationFeedback(evaluationData, passRate);
...
exports.submitQCEvaluation = submitQCEvaluation;
