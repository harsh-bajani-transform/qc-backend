...
const processExcelFiles = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (contentType.includes("multipart/form-data")) {
        return upload.single("file")(req, res, (err) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const projectId = Number((_a = mreq.body) === null || _a === void 0 ? void 0 : _a.project_id);
            const taskId = Number((_b = mreq.body) === null || _b === void 0 ? void 0 : _b.task_id);
            const userId = Number((_c = mreq.body) === null || _c === void 0 ? void 0 : _c.user_id);
            const filePath = ((_d = mreq.body) === null || _d === void 0 ? void 0 : _d.file_path) || null;
            const validateOnly = ((_e = mreq.body) === null || _e === void 0 ? void 0 : _e.validate_only) === "true";
            ...
            try {
                ...
                const rawImportant = (_h = taskRows === null || taskRows === void 0 ? void 0 : taskRows[0]) === null || _h === void 0 ? void 0 : _h.important_columns;
                if (Array.isArray(rawImportant)) {
                    importantColumns = rawImportant;
                }
                else if (typeof rawImportant === "string" &&
                    rawImportant.trim().length > 0) {
                    try {
                        const parsed = JSON.parse(rawImportant);
                        if (Array.isArray(parsed))
                            importantColumns = parsed;
                        else
                            importantColumns = rawImportant
                                .split(",")
                                .map((c) => c.trim())
                ...
                for (const worksheet of workbook.worksheets) {
                    ...
                    worksheet.getRow(1).eachCell((cell, colNumber) => {
                        var _a;
                        headers[colNumber - 1] =
                            ((_a = cell.value) === null || _a === void 0 ? void 0 : _a.toString()) || `col_${colNumber}`;
                    });
                    const allHashes = [];
                    const rowHashes = [];
                    for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
                        ...
                        const colsToUse = importantColumns.length > 0 ? importantColumns : headers;
                        const hashInput = colsToUse
                            ...
                            .toLowerCase()
                            .trim();
                        const hashValue = crypto_1.default
                            .createHash("sha256")
                            .update(hashInput)
                            .digest("hex");
    ...
    try {
        ...
        const existingFiles = trackersWithFiles.filter((tracker) => {
            const resolvedFilePath = path_resolver_1.PathResolver.resolveFilePath(tracker.tracker_file);
            const exists = fs_1.default.existsSync(resolvedFilePath);
            if (!exists) {
                ...
                path_resolver_1.PathResolver.debugFilePath(tracker.tracker_file);
            }
            return exists;
        });
           ...
           WHERE t.task_id = ?`, [tracker.task_id]));
                ...
                if (ext === ".xlsx") {
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                                ...
                                tracker.task_id,
                                tracker.tracker_id,
                                path_1.default.basename(resolvedFilePath),
                   ...
                   WHERE id = ?`, [
                            ...
                            return {
                                processed: true,
                                filename: path_1.default.basename(resolvedFilePath),
                    ...
                    console.warn(`File '${path_1.default.basename(resolvedFilePath)}' is an old Excel format (.xls) and cannot be processed with ExcelJS. Please convert to .xlsx.`);
                    return {
                        processed: false,
                        filename: path_1.default.basename(resolvedFilePath),
                        reason,
