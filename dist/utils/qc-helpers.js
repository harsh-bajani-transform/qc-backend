 ...
 */
function uploadSampleToCloudinary(qc_file_records_1, whole_file_path_1) {
    return __awaiter(this, arguments, void 0, function* (qc_file_records, whole_file_path, percentage = 10, folderName = "hrms/qc_samples") {
        try {
            ...
            if (Array.isArray(sampleData) && sampleData.length > 0) {
                ...
                const buffer = (yield sampleWorkbook.xlsx.writeBuffer());
                const fileName = path_1.default.basename(whole_file_path || "sample", path_1.default.extname(whole_file_path || ".xlsx")) +
                    ...
                    Date.now() +
                    ".xlsx";
                const uploadRes = yield (0, cloudinary_utils_1.uploadBufferToCloudinary)(buffer, folderName, fileName);
 ...
 */
function getQCRecordEmailDetails(connection, agent_id, project_id, task_id, qa_user_id) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        try {
            console.log(`[QC Helper] Fetching email details for agent_id: ${agent_id}`);
            const [agentRows] = yield connection.execute("SELECT user_name, user_email FROM tfs_user WHERE user_id = ?", [agent_id]);
            const [projectRows] = yield connection.execute("SELECT project_name FROM project WHERE project_id = ?", [project_id]);
            const [taskRows] = yield connection.execute("SELECT task_name FROM task WHERE task_id = ?", [task_id]);
            const [qaRows] = yield connection.execute("SELECT user_name FROM tfs_user WHERE user_id = ?", [qa_user_id]);
 ...
 */
function handleQCStatusTransitions(connection, status, agent_id, project_id, task_id, whole_file_path, tracker_id, qcId) {
    return __awaiter(this, void 0, void 0, function* () {
  ...
  `;
        yield connection.execute(deleteTrackerRecordsSql, [
            agent_id,
            project_id,
            task_id,
            whole_file_path,
