  ...
  try {
    ...
    for (const tracker of trackersWithFiles) {
      ...
      try {
        ...
        console.error(
          `Error fetching task info for task ${tracker.task_id}:`,
          error,
        );
        tracker.task_info = {
          task_id: tracker.task_id,
    ...
    for (const tracker of trackersWithFiles) {
      ...
      console.log(
        `Important Columns: ${tracker.task_info.important_columns.join(", ")}`,
      );
          ...
          if (ext === ".xlsx" || ext === ".xls") {
            ...
            try {
              ...
              workbook.eachSheet((worksheet, sheetId) => {
                console.log(
                  `Sheet ${sheetId}: ${worksheet.name} (${worksheet.rowCount} rows × ${worksheet.columnCount} columns)`,
                );
              });
            } catch (excelError) {
              console.log(
