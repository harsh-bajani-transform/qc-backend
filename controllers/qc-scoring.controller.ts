...
export const submitCategoryBasedEvaluation = async (req: Request, res: Response) => {
  try {
    const { 
      user_id, 
      project_id,
    ...
    const validation = ScoringCalculator.validateMarkings(markings);
    ...
    try {
      ...
      await connection.execute(
        ...
        [
          scoringResult.total_percentage,
          user_id,
          overall_notes || ScoringCalculator.generateScoringSummary(scoringResult),
          qc_performance_id
        ]
      );
      ...
      res.status(200).json({
        ...
        data: {
          ...
          summary: ScoringCalculator.generateScoringSummary(scoringResult),
          qc_agent: {
...
export const getEvaluationResults = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const [evaluation] = await connection.execute(
        `SELECT * FROM qc_category_evaluations 
      ...
      // Get category scores
      const [categoryScores] = await connection.execute(
        `SELECT * FROM qc_category_scores 
         WHERE evaluation_id = ?`,
        [evalData.id]
      ) as [any[], any];

      // Get subcategory markings for each category
      const categoryScoresWithDetails = await Promise.all(
        categoryScores.map(async (category: any) => {
          const [subcategories] = await connection.execute(
            `SELECT * FROM qc_subcategory_markings 
             WHERE category_score_id = ?`,
            [category.id]
          ) as [any[], any];
